...
static esp_err_t panel_ili9881c_invert_color(esp_lcd_panel_t *panel,
                                             bool invert_color_data);
static esp_err_t panel_ili9881c_mirror(esp_lcd_panel_t *panel, bool mirror_x,
                                       bool mirror_y);
static esp_err_t panel_ili9881c_disp_on_off(esp_lcd_panel_t *panel,
                                            bool on_off);
static esp_err_t panel_ili9881c_sleep(esp_lcd_panel_t *panel, bool sleep);

esp_err_t
esp_lcd_new_panel_ili9881c(const esp_lcd_panel_io_handle_t io,
                           const esp_lcd_panel_dev_config_t *panel_dev_config,
                           esp_lcd_panel_handle_t *ret_panel) {
  ESP_RETURN_ON_FALSE(io && panel_dev_config && ret_panel, ESP_ERR_INVALID_ARG,
                      TAG, "invalid arguments");
  ili9881c_vendor_config_t *vendor_config =
      (ili9881c_vendor_config_t *)panel_dev_config->vendor_config;
  ...
  ili9881c_panel_t *ili9881c =
      (ili9881c_panel_t *)calloc(1, sizeof(ili9881c_panel_t));
  ESP_RETURN_ON_FALSE(ili9881c, ESP_ERR_NO_MEM, TAG,
                      "no mem for ili9881c panel");
  ...
  // --- REPLACED LEGACY I2C WITH NEW MASTER API ---
  if (i2c1_bus == NULL) {
    ESP_LOGE(TAG, "I2C1 Bus not initialized! Display power might fail.");
  } else {
    ...
    i2c_master_dev_handle_t pwr_chip_handle;
    if (i2c_master_bus_add_device(i2c1_bus, &dev_cfg, &pwr_chip_handle) ==
        ESP_OK) {
      uint8_t data[2];

      data[0] = 0x95;
      data[1] = 0x11;
      i2c_master_transmit(pwr_chip_handle, data, 2, 100);
      ...
      i2c_master_bus_rm_device(pwr_chip_handle);
    }
  }
  // --- END OF REPLACEMENT ---
...
static const ili9881c_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    {ILI9881C_CMD_CNDBKxSEL,
     ...
     3, 0},
    // {0x3A, (uint8_t[]){0x77}, 1, 0}, // REMOVED: Managed dynamically by
    // esp_lcd_panel_colmod
    // {0x36, (uint8_t[]){0x00}, 1, 0}, // REMOVED: Managed dynamically by
    // esp_lcd_panel_madctl
    {0x35, (uint8_t[]){0x00}, 1, 0},
    {0x35, (uint8_t[]){0x00}, 1, 0},
...
static esp_err_t panel_ili9881c_init(esp_lcd_panel_t *panel) {
  ...
  uint8_t ID1, ID2, ID3;
  esp_lcd_panel_io_tx_param(io, ILI9881C_CMD_CNDBKxSEL,
                            (uint8_t[]){ILI9881C_CMD_BKxSEL_BYTE0,
                                        ILI9881C_CMD_BKxSEL_BYTE1,
                                        ILI9881C_CMD_BKxSEL_BYTE2_PAGE1},
                            3);
  ...
  if (ili9881c->init_cmds) {
    ...
    init_cmds_size =
        sizeof(vendor_specific_init_default) / sizeof(ili9881c_lcd_init_cmd_t);
  }
