...
// Helper function to update gauge value, text, and color
static void update_gauge(lv_obj_t *arc, lv_obj_t *label, float value,
                         const char *fmt, float warn_thr, float crit_thr,
  ...
  // Check thresholds
  bool is_warn = invert_logic ? (value <= warn_thr && value > crit_thr)
                              : (value >= warn_thr && value < crit_thr);
...
void update_all_gauges(void) {
  ...
  update_gauge(ui_Arc_MAP, ui_Label_MAP_Value, data.map_kpa, "%.0f", 1500, 1800,
               false, lv_color_hex(0x00D4FF));

  // RPM: Cyan. Warn: 7500, Crit: 9000
  update_gauge(ui_Arc_RPM, ui_Label_RPM_Value, data.engine_rpm, "%.0f", 7500,
               9000, false, lv_color_hex(0x00D4FF));

  // TPS: Cyan. Warn: 80, Crit: 90
  update_gauge(ui_Arc_TPS, ui_Label_TPS_Value, data.tps_position, "%.1f", 80,
               90, false, lv_color_hex(0x00D4FF));

  // Wastegate: Cyan. Warn: 110, Crit: 120
  update_gauge(ui_Arc_Wastegate, ui_Label_Wastegate_Value, data.wg_pos_percent,
               "%.1f", 110, 120, false, lv_color_hex(0x00D4FF));

  // Boost (Target): Cyan.
  if (ui_Arc_Boost && ui_Label_Boost_Value) {
    update_gauge(ui_Arc_Boost, ui_Label_Boost_Value, data.map_kpa, "%.0f", 200,
                 230, false, lv_color_hex(0x00D4FF));
  }

  // --- Screen 2 ---
  // Oil Pressure: Orange (0xFF6B35). Warn: < 2.0, Crit: < 1.0
  update_gauge(ui_Arc_Oil_Pressure, ui_Label_Oil_Pressure_Value, 0, "%.1f", 2.0,
               1.0, true, lv_color_hex(0xFF6B35));

  // Oil Temp: Gold (0xFFD700). Warn: 110, Crit: 120
  update_gauge(ui_Arc_Oil_Temp, ui_Label_Oil_Temp_Value, data.oil_temp, "%.0f",
               110, 120, false, lv_color_hex(0xFFD700));

  // Water Temp: Cyan (0x00D4FF). Warn: 105, Crit: 115
  update_gauge(ui_Arc_Water_Temp, ui_Label_Water_Temp_Value, data.clt_temp,
               "%.0f", 105, 115, false, lv_color_hex(0x00D4FF));

  // Fuel Pressure: Green (0x00FF88). Warn: < 3.0, Crit: < 2.0
  update_gauge(ui_Arc_Fuel_Pressure, ui_Label_Fuel_Pressure_Value, 0, "%.1f",
               3.0, 2.0, true, lv_color_hex(0x00FF88));

  // Battery: Gold (0xFFD700). Warn: < 12.0, Crit: < 11.5
  update_gauge(ui_Arc_Battery_Voltage, ui_Label_Battery_Voltage_Value,
               data.battery_voltage, "%.1f", 12.0, 11.5, true,
               lv_color_hex(0xFFD700));

  // --- Screen 4 ---
  // Abs Pedal: Cyan
  update_gauge(ui_Arc_Abs_Pedal, ui_Label_Abs_Pedal_Value, data.abs_pedal_pos,
               "%.1f", 110, 120, false, lv_color_hex(0x00D4FF));

  // WG Pos: Green
  update_gauge(ui_Arc_WG_Pos, ui_Label_WG_Pos_Value, data.wg_pos_percent,
               "%.1f", 110, 120, false, lv_color_hex(0x00FF88));

  // BOV: Gold
  update_gauge(ui_Arc_BOV, ui_Label_BOV_Value, data.bov_percent, "%.1f", 110,
               120, false, lv_color_hex(0xFFD700));

  // TCU TQ: Orange
  update_gauge(ui_Arc_TCU_TQ_Req, ui_Label_TCU_TQ_Req_Value, data.tcu_tq_req_nm,
               "%.0f", 450, 500, false, lv_color_hex(0xFF6B35));
  update_gauge(ui_Arc_TCU_TQ_Act, ui_Label_TCU_TQ_Act_Value, data.tcu_tq_act_nm,
               "%.0f", 450, 500, false, lv_color_hex(0xFF3366));
  update_gauge(ui_Arc_Eng_TQ_Req, ui_Label_Eng_TQ_Req_Value, data.eng_trg_nm,
               "%.0f", 450, 500, false, lv_color_hex(0x8A2BE2));

  // --- Screen 5 ---
  // Eng Tq Act: Cyan
  update_gauge(ui_Arc_Eng_TQ_Act, ui_Label_Eng_TQ_Act_Value, data.eng_act_nm,
               "%.0f", 450, 500, false, lv_color_hex(0x00D4FF));
  // Limit Tq: Green
  update_gauge(ui_Arc_Limit_TQ, ui_Label_Limit_TQ_Value, data.limit_tq_nm,
  ...
  // RPM (Left) - Red
  update_gauge(ui_Gauge_RPM_S8, ui_Label_RPM_Val_S8, data.engine_rpm, "%.0f",
  ...
  update_gauge(ui_Gauge_Speed_S8, ui_Label_Speed_Val_S8, data.vehicle_speed,
